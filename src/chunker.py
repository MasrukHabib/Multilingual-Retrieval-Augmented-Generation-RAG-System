...
def recursive_character_text_splitter(text: str, chunk_size: int, chunk_overlap: int) -> List[str]:
    ...
    # 1. Sentence endings (Bangla punctuation: ।, ?, !) followed by whitespace
    ...
    def _split_recursively(segment: str, current_separators: List[str]) -> List[str]:
        ...
        for i, part in enumerate(parts):
            ...
            if i < len(parts) - 1 and separator_pattern != "":
                # For sentence/paragraph separators, add a single space or newline back
            ...
            if len(part) > chunk_size:
                # If a part is still too large, recursively split it with the next separator
    ...
    # Now, combine raw_chunks into final chunks respecting chunk_size and chunk_overlap
    final_chunks = []
    current_chunk_content = ""

    for part in raw_chunks:
        # Check if adding the new part exceeds the chunk size
        # Account for potential overlap from the previous chunk
        if len(current_chunk_content) + len(part) + (len(final_chunks) > 0 and chunk_overlap > 0) > chunk_size:
            # If current_chunk_content is not empty, add it to final_chunks
    ...
    # Final pass to ensure no chunk exceeds chunk_size and handle any remaining large chunks
    # This loop is crucial for cases where a single 'part' from raw_chunks (e.g., a very long sentence
    # or word if character-level splitting was the only option) is larger than chunk_size.
    refined_chunks = []
...
if __name__ == '__main__':
    ...
    তৃতীয় পরিচ্ছেদ: বাংলা ব্যাকরণ। বাংলা ব্যাকরণ ভাষার নিয়মকানুন নিয়ে আলোচনা করে। এর প্রধান আলোচ্য বিষয়গুলো হলো ধ্বনিতত্ত্ব, রূপতত্ত্ব, বাক্যতত্ত্ব এবং অর্থতত্ত্ব। ধ্বনিতত্ত্বে ধ্বনি ও বর্ণ নিয়ে আলোচনা করা হয়। রূপতত্ত্বে শব্দ ও পদ নিয়ে আলোচনা করা হয়। অনুপম তার মামার কাছে শুম্ভুনাথকে সুপুরুষ বলে বর্ণনা করেছে। শুম্ভুনাথের স্বাস্থ্য ছিল সুঠাম এবং তার মুখশ্রী ছিল শান্ত ও গম্ভীর।
    """
    ...
    for i, chunk in enumerate(chunks2):
        ...
        if "অনুপম তার মামার কাছে শুম্ভুনাথকে সুপুরুষ" in chunk:
